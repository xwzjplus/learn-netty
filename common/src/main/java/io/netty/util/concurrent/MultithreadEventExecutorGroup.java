...
public abstract class MultithreadEventExecutorGroup extends AbstractEventExecutorGroup {
    ...
    //nThreads如果不传默认是0 如果是0的话就获取CPU核数的两倍 DEFAULT_EVENT_LOOP_THREADS==CPU核数的两倍
    //executor默认为null
    //chooserFactory=new DefaultEventExecutorChooserFactory() 默认 事件执行策略工厂

    //args参数如下
    //SelectorProvider ServerSocketChannel就是通过ServerSocketChannel.open()==>SelectorProvider.provider().openServerSocketChannel()创建的
    //DefaultSelectStrategyFactory.INSTANCE ===> new DefaultSelectStrategyFactory() select轮训的策略
    //RejectedExecutionHandlers.reject() ===> new RejectedExecutionHandler() 线程池拒绝策略
